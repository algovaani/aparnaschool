...
class Student_model extends MY_Model
{
    ...
    public function get($id = null)
    {
        ...
        $this->db->select('pickup_point.name as pickup_point_name,student_session.route_pickup_point_id,student_session.transport_fees,students.app_key,students.parent_app_key,student_session.vehroute_id,vehicle_routes.route_id,vehicle_routes.vehicle_id,transport_route.route_title,vehicles.vehicle_no,hostel_rooms.room_no,vehicles.driver_name,vehicles.driver_contact,vehicles.vehicle_model,vehicles.manufacture_year,vehicles.driver_licence,vehicles.vehicle_photo,hostel.id as `hostel_id`,hostel.hostel_name,room_types.id as `room_type_id`,room_types.room_type ,students.hostel_room_id,student_session.id as `student_session_id`,student_session.fees_discount,classes.id AS `class_id`,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no,students.roll_no,students.admission_no,students.admission_date,students.firstname,students.middlename,  students.lastname,students.image,students.mobileno, students.email ,students.state,students.city,students.pincode,students.note,students.religion,students.cast, school_houses.house_name,students.dob,students.current_address,students.previous_school,           students.guardian_is,students.parent_id,  students.permanent_address,students.category_id,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name , students.father_pic ,students.height ,students.weight,students.measurement_date, students.mother_pic , students.guardian_pic , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.father_phone,students.blood_group,students.school_house_id,students.father_occupation,students.mother_name,students.mother_phone,students.mother_occupation,students.guardian_occupation,students.gender,students.guardian_is,students.rte,students.guardian_email, users.username,users.password,users.id as user_id,students.subject_name,students.class_date,students.crft_sub,students.core_sub,students.last_fee_paid_date,students.first_fee_paid_date,students.concession_reciept,students.ncc_cadet,students.game_play,students.cunduct,students.anul_chrg_colct,students.tot_school_days,students.dis_reason,students.dis_note,students.disable_at,students.pupil_attend_days')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('hostel_rooms', 'hostel_rooms.id = students.hostel_room_id', 'left');
    ...
    public function admission_reportNew($searchterm, $carray = null, $condition = null)
{
    $class_section_array = $this->customlib->get_myClassSection();
    $userdata = $this->customlib->getUserData();
    ...
    // Main SELECT with fee information
    $this->datatables->select('
        ...
        (
            SELECT SUM(fee_groups_feetype.amount)
            ...
            WHERE student_fees_master.student_session_id = student_session.id
            AND feetype.type LIKE "%ADMISSION%"
        ) AS admission_fee_amount,
        (
            SELECT GROUP_CONCAT(fd.amount_detail SEPARATOR \'||\')
            FROM student_fees_deposite fd
            INNER JOIN student_fees_master sfm ON sfm.id = fd.student_fees_master_id
            INNER JOIN fee_groups_feetype fgf ON fgf.id = fd.fee_groups_feetype_id
            WHERE sfm.student_session_id = student_session.id
            AND fgf.session_id = student_session.session_id
        ) AS fee_details'
        . (!empty($field_variable) ? ',' . $field_variable : '')
    );
    ...
    // Filter by class/section access
    if (!empty($class_section_array)) {
        $this->datatables->group_start();
        foreach ($class_section_array as $class_sectionkey => $class_sectionvalue) {
            foreach ($class_sectionvalue as $section_id) {
                $this->datatables->or_where("( student_session.class_id={$class_sectionkey} AND student_session.section_id={$section_id} )");
            }
        }
        $this->datatables->group_end();
    }
    ...
    //             WHERE student_fees_master.student_session_id = student_session.id
    //             AND feetype.type LIKE "%ADMISSION%"
    //         ) AS admission_fee_amount' . (!empty($field_variable) ? ',' . $field_variable : '')
    //     );
    ...
    public function getStudentsByArray($array)
    {
        ...
        $this->db->select('classes.id AS `class_id`,student_session.id as student_session_id,students.id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no,students.roll_no,students.admission_date,students.firstname, students.middlename,students.lastname,students.image,students.mobileno,students.email,students.state,students.city,students.pincode,students.religion,     students.dob ,students.current_address,students.blood_group,students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.cast,students.bank_name, students.ifsc_code,students.guardian_name, students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.mother_name,students.updated_at,students.father_name,students.rte,students.gender,users.id as `user_tbl_id`,users.username,users.password as `user_tbl_password`,users.is_active as `user_tbl_active`,school_houses.house_name,students.tot_school_days,students.anul_chrg_colct,students.cunduct,students.game_play,students.ncc_cadet,students.concession_reciept,students.last_fee_paid_date,students.first_fee_paid_date,students.crft_sub,students.core_sub,' . $field_variable)->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('categories', 'students.category_id = categories.id', 'left');
