...
class Section_model extends MY_Model {
    ...
    public function get($id = null) {
        ...
        // Check if user is director with role_id 19
        if (isset($role_id) && isset($user_type) && $role_id == 19 && $user_type == 'DIRECTOR') {
            // Return only section "JW" for director users
            $this->db->select()->from('sections');
            if ($id != null) {
                $this->db->where('id', $id);
            } else {
                $this->db->where('section', 'JW');
                $this->db->order_by('id');
            }
            $query = $this->db->get();
            if ($id != null) {
...
//     }
    ...
    public function getClassBySection($classid) {
        ...
        // Case 1: If user is a teacher with restricted sections
        if ($role_id == 2 && ($userdata["class_teacher"] ?? '') == "yes") {
            $section = $this->teacher_model->get_teacherrestricted_modesections($userdata["id"], $classid);
    
        // Case 2: If user is DIRECTOR (role_id = 19)
        } elseif ($role_id == 19 && $user_type == 'DIRECTOR') {
            $this->db->select('class_sections.id,class_sections.section_id,sections.section');
            $this->db->from('class_sections');
            $this->db->join('sections', 'sections.id = class_sections.section_id');
            $this->db->where('class_sections.class_id', $classid);
    
            // ✅ Nursery से Class 03 तक केवल "JW" section दिखे
            if (in_array($classid, ['NURSERY', '01', '02', '03'])) {
                $this->db->where('sections.section', 'JW');
            }
        ...
        // Case 3: For all other roles (default behaviour)
        } else {
            $this->db->select('class_sections.id,class_sections.section_id,sections.section');
            $this->db->from('class_sections');
            $this->db->join('sections', 'sections.id = class_sections.section_id');
