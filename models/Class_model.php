...
class Class_model extends MY_Model
{
    ...
    public function getAll($id = null)
    {
        ...
        $classlist   = array();
        if ($role_id == 19 && $user_type == 'DIRECTOR') {
            ...
            $query = $this->db->get();
            $all_classes = $query->result_array();
            // Filter classes to only include Nursery to 03
            foreach ($all_classes as $class) {
                $class_name = trim($class['class']);
                if ($this->isTargetClass($class_name)) {
                    $classlist[] = $class;
                }
            }
        }else{
    ...
    private function isTargetClass($class_name)
    {
        // Check for exact matches
        if (in_array($class_name, ['NURSERY','Nursery', 'LKG', 'UKG', '01', '02', '03', '1', '2', '3'])) {
            return true;
        }
        // Check for numeric classes
        elseif (is_numeric($class_name) && intval($class_name) >= 1 && intval($class_name) <= 3) {
            return true;
        }
        // Check for variations like "Class 1", "Grade 1", etc.
        elseif (preg_match('/^(class|grade|std|standard)\s*(\d+)$/i', $class_name, $matches)) {
    ...
    public function get($id = null, $classteacher = null)
    {
        ...
        }else if ($role_id == 19 && $user_type == 'DIRECTOR') {
            $this->db->select()->from('classes');
            if ($id != null) {
                $this->db->where('id', $id);
            } else {
