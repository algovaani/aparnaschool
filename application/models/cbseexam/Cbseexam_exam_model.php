    ...
    */
    ...
    public function addresultmark_data($result_mark, $cbse_exam_timetable_id)
    {
        ...
        if (isset($result_mark['id']) && !empty($result_mark['id'])) {
            ...
            // Collect student IDs and assessment type IDs for this batch
            foreach ($result_mark as $mark_key => $mark_value) {
                $student_ids[] = $mark_value['cbse_exam_student_id'];
                $assessment_type_ids[] = $mark_value['cbse_exam_assessment_type_id'];
            }
            ...
            // Delete existing records for these specific students and assessment types only
            if (!empty($student_ids) && !empty($assessment_type_ids)) {
                $this->db->where('cbse_exam_timetable_id', $cbse_exam_timetable_id);
                $this->db->where_in('cbse_exam_student_id', $student_ids);
                $this->db->where_in('cbse_exam_assessment_type_id', $assessment_type_ids);
                $this->db->delete('cbse_student_subject_marks');
            }
