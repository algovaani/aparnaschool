...
class Studentfee_model extends MY_Model {
         ...
         if (!empty($feegroups))
  {
    ...
    /**
     * Get paid amount from amount_detail JSON (student_fees_deposite)
     */
    private function _getPaidFromAmountDetail($amount_detail, $fee_amount) {
        $paid = 0;
        if (empty($amount_detail) || $amount_detail == '0') {
            return 0;
        }
        $detail = is_string($amount_detail) ? json_decode($amount_detail, true) : $amount_detail;
        if (!is_array($detail)) {
            return 0;
        }
        foreach ($detail as $d) {
    ...
    /**
     * Students with fees due for 60+ days: list with months unpaid and total due amount
     * Returns array of students with: id, admission_no, name, class, section, guardian_name, mobileno, months_unpaid, total_due, days_overdue
     */
    public function getStudentsWithDueFeesOver60Days($class_id = null, $section_id = null) {
        $cutoff_date = date('Y-m-d', strtotime('-60 days'));
        $rows = $this->getDueStudentFeesByDateClassSection($class_id, $section_id, $cutoff_date);
        $by_student = array();
        $today_ts = strtotime(date('Y-m-d'));
        foreach ($rows as $row) {
            $amount = isset($row['amount']) ? floatval($row['amount']) : (isset($row['previous_balance_amount']) ? floatval($row['previous_balance_amount']) : 0);
            $amount_detail = isset($row['amount_detail']) ? $row['amount_detail'] : null;
            $paid = $this->_getPaidFromAmountDetail($amount_detail, $amount);
            ...
            if ($due_date && $due_date != '0000-00-00') {
                $days_overdue = floor(($today_ts - strtotime($due_date)) / 86400);
            }
            if (!isset($by_student[$student_id])) {
                $by_student[$student_id] = array(
                    'id' => $row['id'],
            ...
            $by_student[$student_id]['months_unpaid'] += 1;
            $by_student[$student_id]['total_due'] += $due_amount;
            if ($days_overdue > $by_student[$student_id]['days_overdue']) {
                $by_student[$student_id]['days_overdue'] = $days_overdue;
            }
        }
        return array_values($by_student);
    }
