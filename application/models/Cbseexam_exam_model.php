...
class Cbseexam_exam_model extends MY_Model
{
    ...
    */
    ...
    public function getExamStudents($exam_id)
    {
        ...
        $this->db->where('student_session.session_id', $this->current_session);
        $this->db->where('students.is_active', 'yes');
        $this->db->order_by('cbse_student_exam_ranks.rank','ASC');
    ...
    {
        $result = array();
        $this->db->select('students.*,cbse_exam_students.id as exam_student_id,cbse_exam_timetable.id as cbse_exam_timetable_id,cbse_exam_students.roll_no as `exam_roll_no`,classes.class as class_name,sections.section as section_name,student_session.class_id,student_session.section_id')
            ->from('cbse_exam_students')
            ->join('student_session', 'student_session.id=cbse_exam_students.student_session_id')
            ->join('classes', 'student_session.class_id = classes.id')
            ->join('sections', 'sections.id = student_session.section_id')
    ...
    public function addresultmark_data($result_mark, $cbse_exam_timetable_id)
    {
        ...
        if (isset($result_mark['id']) && !empty($result_mark['id'])) {
            ...
            $insert_ids = [];
            $student_ids = []; // Track student IDs to avoid cross-section deletion
            ...
            foreach ($result_mark as $mark_key => $mark_value) {
                ...
                // Collect student IDs to ensure we only delete marks for students being updated
                if (isset($mark_value['cbse_exam_student_id'])) {
                    $student_ids[] = $mark_value['cbse_exam_student_id'];
                }
            }
            
            // FIXED: Only delete marks for the specific students being updated, not all students in the timetable
            if (!empty($insert_ids) && !empty($student_ids)) {
                $this->db->where('cbse_exam_timetable_id', $cbse_exam_timetable_id);
                $this->db->where_in('cbse_exam_student_id', $student_ids); // Only delete for specific students
