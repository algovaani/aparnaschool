...
<td class="text text-right">
<?php
if (!empty($fee_value->amount_detail) && ($fee_discount == $fee_value->amount)){
    echo amountFormat($fee_value->amount);
    $total_deposite_amount = $total_deposite_amount+($fee_value->amount);
}else if (!empty($fee_value->amount_detail) && ($fee_discount != $fee_value->amount)){
    echo amountFormat($fee_value->amount-$fee_discount);
    $total_deposite_amount = $total_deposite_amount+($fee_value->amount-$fee_discount);
}  else {
    echo "0.00";
    
    // echo "VB".amountFormat($fee_value->amount-$fee_discount);
}
?>                                                      
...
if (empty($fee_value->amount_detail)) {
    echo "0.00";
}else if ($fee_discount == $fee_value->amount){
echo amountFormat($fee_discount);
} else {
...
$display_none = "ss-none";
            if (empty($fee_value->amount_detail)) {
                ...
                $display_none = "";
                $paid = $fee_value->amount-$fee_discount;
                echo amountFormat($fee_value->amount-($paid+$fee_discount));//amountFormat($feetype_balance);
            }
            ?>
                                                        </td>
                                                    </tr>
...
echo amountFormat($fee_fine);
            ?></td>
                                                <td class="text text-right"><?php
                                                if(!empty($fee_value->amount_detail) && (amountFormat($fee_discount) == $transport_fee_value->fees)){
                                                    echo (amountFormat($transport_fee_value->fees)-amountFormat($fee_discount));
                                                }else{
                                                    echo "0.00";
                                                }
...
$display_none = "ss-none";
            if ($feetype_balance > 0) {
                ...
                // echo amountFormat($feetype_balance);
                $paidAmount = amountFormat($transport_fee_value->fees)-amountFormat($fee_discount);
                echo amountFormat($transport_fee_value->fees)-(amountFormat($fee_discount)+amountFormat($paidAmount));
            }
            ?>
                                                </td>
                                            </tr>
...
if ($total_fees_fine_amount != "") {
    ...
    ?>
                                                ...
                                                <td></td>
                                                <td class="text text-right"><?php
                                                $totalPaid =  $total_amount - ($total_discount_amount + $alot_fee_discount);
                                                $balancePaid = ($total_amount -($total_discount_amount + $alot_fee_discount+$total_deposite_amount));
...
echo ($currency_symbol . amountFormat($balancePaid));
