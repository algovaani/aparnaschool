...
<head>
    ...
    <style type="text/css">
        ...
        .attendance-tab-container {
            ...
            padding-bottom: 5px; /* Ensures space for scrollbar */
        }
        ...
        .box { border-radius: 3px; background: #ffffff; border-top: 3px solid #d2d6de; margin-bottom: 20px; width: 100%; box-shadow: 0 1px 1px rgba(0,0,0,0.1); }
        ...
        .noteinput { width: 100%; box-sizing: border-box; padding: 6px 8px; font-size: 13px; line-height: 1.42857143; color: #555; background-color: #fff; background-image: none; border: 1px solid #ccc; border-radius: 4px; box-shadow: inset 0 1px 1px rgba(0,0,0,.075); transition: border-color ease-in-out .15s,box-shadow ease-in-out .15s; }
        ...
        /* --- Mobile Specific Styles (Card View and form layout) --- */
        @media (max-width:767px){
            .desktop-only-view {
                display: none; /* Hide the entire table on mobile */
            }
            ...
            /* Adjust columns for small screens within the criteria form */
            .col-md-4 {
                width: 100%; /* Make each column full width on mobile */
                float: none; /* Remove float to allow stacking */
            }
            .col-md-12 {
                width: 100%;
                float: none;
            ...
            .form-group {
                margin-bottom: 10px; /* Reduce margin slightly for tighter packing */
            }
            .pull-right {
                float: none !important; /* Remove float for better stacking of buttons */
                display: block; /* Make button full width if needed, or inline-block for its container */
                text-align: center; /* Center the button if it's block level */
                width: 100%; /* Make button take full width */
            }
            .checkbox-toggle {
                margin-bottom: 15px; /* Add some space below the holiday checkbox */
            }
            ...
            .card-bg-1 { background-color: #e6f2ff; /* Light Blue */ }
            .card-bg-2 { background-color: #e6ffe6; /* Light Green */ }
            .card-bg-3 { background-color: #fffde6; /* Light Yellow */ }
            .card-bg-4 { background-color: #ffe6f2; /* Light Pink */ }
            .card-bg-5 { background-color: #f2f2f2; /* Light Grey */ }
            ...
            .student-card-mobile .card-row.student-info-row {
                ...
                gap: 10px; /* Reduced gap between these specific items */
            }
            ...
            .student-card-mobile .card-label {
                ...
                display: inline; /* Keep label inline with value by default */
                margin-right: 4px; /* Small space between label and value */
            }

            /* Make Admission No., Roll No., and Name labels bold in mobile cards */
            .student-card-mobile .card-label.bold-label {
                font-weight: bold !important; /* Explicitly bold for these labels */
            }
            
            /* Styling for the values themselves in mobile view */
            .student-card-mobile .card-value {
                font-size: 14px; /* Default value font size */
                color: #333;
            }
            ...
            .student-card-mobile .card-row.full-width-item .card-item .card-value {
                display: inline-block; /* Make label and value sit side-by-side */
                vertical-align: middle; /* Align them nicely */
            }
            /* Add margin for label in full-width items */
            .student-card-mobile .card-row.full-width-item .card-item .card-label {
                margin-bottom: 0; /* Remove previous block margin */
                margin-right: 5px; /* Small gap between label and value */
            }
...
<div class="content-wrapper" style="min-height: 946px;">
                            ...
                            <div class="box-body">
                                ...
                                if (!empty($resultlist)) {
                                        ...
                                        if (!($this->session->flashdata('msg'))) {
                                            if (isset($resultlist[0]) && $resultlist[0]['attendence_type_id'] != "") {
                                                if ($resultlist[0]['attendence_type_id'] != 5) { // Assuming 5 is the ID for Holiday
                                                    ...
                                                    <table class="table table-hover table-striped example">
                                                        ...
                                                        <tbody>
                                                            ...
                                                            foreach ($resultlist as $key => $value) {
                                                                $initial_attendance_type_id = isset($value['attendence_type_id']) ? $value['attendence_type_id'] : null;
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        <div class="attendance-tab-container">
                                                                            ...
                                                                            ?>
                                                                                    <input type="radio" id="attendencetype_desktop<?php echo $value['student_session_id'] . "-" . $count; ?>" value="<?php echo $type['id'] ?>" name="attendencetype<?php echo $value['student_session_id']; ?>" class="hidden_radio" <?php echo $is_checked; ?>>
                                                                                    <span class="attendance-tab <?php echo $active_class; ?>" data-radio-id="attendencetype_desktop<?php echo $value['student_session_id'] . "-" . $count; ?>" data-type="<?php echo $att_type_key; ?>">
                                                                    ...
                                                                    <td class="noteinput">
                                                                        <input type="text" class="noteinput" name="remark<?php echo $value["student_session_id"] ?>" value="<?php echo (isset($value["remark"]) ? $value["remark"] : ""); ?>" >
                                                                    </td>
                                                                </tr>
                                                            <?php
                                                                $row_count++;
                                                    ...
                                                    ?>
                                                        <div class="student-card-mobile card-bg-<?php echo ($row_count % 5) + 1; ?>">
                                                            ...
                                                            <div class="card-row student-info-row">
                                                                ...
                                                                <div class="card-item"><span class="card-label bold-label">Admn No.:</span> <span class="card-value admission-no-value"><?php echo $value['admission_no']; ?></span></div>
                                                                <div class="card-item"><span class="card-label bold-label">Roll No.:</span> <span class="card-value"><?php echo $value['roll_no']; ?></span></div>
                                                            </div>
                                                            <?php if ($sch_setting->biometric) { ?>
                                                            ...
                                                            <div class="card-row full-width-item">
                                                                <div class="card-item">
                                                                    ...
                                                                    <span class="card-value name-value"><?php echo $this->customlib->getFullName($value['firstname'], $value['middlename'], $value['lastname'], $sch_setting->middlename, $sch_setting->lastname); ?></span>
                                                                </div>
                                                            </div>
                                                                ...
                                                                <div class="attendance-tab-container">
                                                                    ...
                                                                    foreach ($attendencetypeslist as $type) {
                                                                        if ($type['key_value'] != "H") {
                                                                            ...
                                                                            $active_class = $is_checked ? "active-" . $att_type_key : "";
                                                                    ?>
                                                                            <input type="radio" id="attendencetype_mobile<?php echo $value['student_session_id'] . "-" . $count; ?>" value="<?php echo $type['id'] ?>" name="attendencetype<?php echo $value['student_session_id']; ?>" class="hidden_radio" <?php echo $is_checked; ?>>
                                                                            <span class="attendance-tab <?php echo $active_class; ?>" data-radio-id="attendencetype_mobile<?php echo $value['student_session_id'] . "-" . $count; ?>" data-type="<?php echo $att_type_key; ?>">
                                                                                <?php echo $this->lang->line($att_type_key); ?>
                                                                            </span>
                                                                    <?php
                                                                            $count++;
                                            ...
                                            <div class="pull-right" style="margin-top: 20px; margin-bottom: 20px;">
                                                ...
                                                if ($can_edit == 1) {
                                                    if ($this->rbac->hasPrivilege('student_attendance', 'can_add')) {
                                                        ?>
                                                        <button type="submit" name="search" value="saveattendence" class="btn btn-primary btn-sm checkbox-toggle"><i class="fa fa-save"></i> <?php echo $this->lang->line('save_attendance'); ?> </button>
                                                        <?php
                                                    }
                                                }
                                                ?>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            // Initialize DataTables (for desktop view only)
            // The table is hidden on mobile via CSS, so DataTables only applies to desktop
            ...
            // If you're using DataTables Buttons extension, uncomment the line below:
            ...
            $(document).on('change', '#class_id', function (e) {
                ...
                var base_url = '<?php echo base_url() ?>';
                var div_data = '<option value=""><?php echo $this->lang->line('select'); ?></option>';
                var url_segment = "getByClass";
                $.ajax({
                    type: "GET",
                    url: base_url + "sections/" + url_segment,
            ...
            $(document).on('click', '.attendance-tab', function() {
                ...
                // Find all tabs within the same student's context (either table row or mobile card)
                var $studentContainer = $this.closest('tr, .student-card-mobile');
                $studentContainer.find('.attendance-tab')
                    ...
                    $checkbox.on('change', function () {
                        ...
                        if (isChecked) {
                            ...
                            $('.hidden_radio').each(function() {
                                ...
                                $tab.removeClass('active-present active-absent active-late active-half_day');
            ...
            // Only run this when in mobile view (or on window resize if you support dynamic switching)
