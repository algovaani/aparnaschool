...
modal_click_disabled('subjectModal','addExamModal','observationParameterModal','teacherRemarkModal','allotStudentModal','addSubject','editexamModal','observationModal','subjectmarkModal');
    ...
    // import marks
        ...
        $(document).on('click', "#btnSubmit", function (event) {
            ...
            $.ajax({
                url: baseurl + "/admin/examgroup/uploadfilecbse",
                type: 'POST',
                dataType: 'JSON',
                data: form_data,
               contentType: false,
          ...
          $.each(data.error, function (index, value) {
           ...
           var arr = [];
            $.each(data.student_marks, function (index) {
                ...
                // Loop to set attands values
                for (var i = 1; i <= data.total_subjects; i++) {
                    dataObj['attands' + i] = s['attands' + i];
                }
                dataObj.note = s.note;
                arr.push(dataObj);
            });
                    ...
                    $.each(arr, function (index, value) {
                         let adm_no_csv =value.adm_no;
                             var row = $('.marksEntryForm').find('table tbody').find('tr.std_adm_' + adm_no_csv);
                                ...
                                row.find("td input.noteinput").val(value.note);
                                ...
                                for (var i = 1; i <= data.total_subjects; i++) {
                                    ...
                                    var fieldName1 = 'attands' + i;
                                    if (value[fieldName1] == 0) {
                                        row.find("td input.attendance_chk"+ i).prop("checked", true);
                                    } else {
                                        row.find("td input.attendance_chk"+ i).prop("checked", false);
                                    }
                                }
                    });
