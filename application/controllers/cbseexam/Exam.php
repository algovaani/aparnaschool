    ...
    public function updateExamRank($exam_id)
{
    ...
    if (!empty($cbse_exam_result)) {
        // Group students by class and section first
        $class_section_students = [];
        
        foreach ($cbse_exam_result as $student_key => $student_value) {
            $class_section_key = $student_value->class_id . '-' . $student_value->section_id;
            
            if (!isset($class_section_students[$class_section_key])) {
                $class_section_students[$class_section_key] = [
            ...
            if (!isset($class_section_students[$class_section_key]['students'][$student_value->student_session_id])) {
                $class_section_students[$class_section_key]['students'][$student_value->student_session_id] = [
            ...
            if (!isset($class_section_students[$class_section_key]['students'][$student_value->student_session_id]['subjects'][$student_value->subject_id])) {
                $class_section_students[$class_section_key]['students'][$student_value->student_session_id]['subjects'][$student_value->subject_id] = [
            ...
            $marks = $student_value->is_absent ? 0 : $student_value->marks;
            $class_section_students[$class_section_key]['students'][$student_value->student_session_id]['subjects'][$student_value->subject_id]['total_marks'] += $marks;
            $class_section_students[$class_section_key]['students'][$student_value->student_session_id]['subjects'][$student_value->subject_id]['total_max_marks'] += $student_value->maximum_marks;
            
            // Update student totals
            $class_section_students[$class_section_key]['students'][$student_value->student_session_id]['total_marks'] += $marks;
            $class_section_students[$class_section_key]['students'][$student_value->student_session_id]['total_max_marks'] += $student_value->maximum_marks;
        }
        
        // Calculate percentages and ranks for each class-section
        $student_allover_rank = [];
        
        foreach ($class_section_students as $class_section_key => $class_section) {
            // Calculate percentages
            foreach ($class_section['students'] as $student_key => $student) {
                $percentage = ($student['total_max_marks'] > 0) ? ($student['total_marks'] / $student['total_max_marks']) * 100 : 0;
                $class_section_students[$class_section_key]['students'][$student_key]['percentage'] = $percentage;
            }
            
            // Sort students by percentage (descending) within this class-section
            usort($class_section_students[$class_section_key]['students'], function($a, $b) {
                return $b['percentage'] <=> $a['percentage'];
            });
            
            // Assign ranks within class-section
            $prev_percentage = null;
            $rank = 0;
            $skip = 0;
            
            foreach ($class_section_students[$class_section_key]['students'] as $index => $student) {
                ...
                $class_section_students[$class_section_key]['students'][$index]['rank'] = $rank;
                ...
                $student_allover_rank[] = [
                    ...
                    'rank_percentage' => $student['percentage'],
                    'rank' => $rank
                ];
            }
        }
        ...
        return $class_section_students; // You can return this for debugging or other purposes
    }
    ...
    public function examwiserank($exam_id)
    {
        ...
        $studentList = $this->cbseexam_exam_model->getExamStudents($exam_id);

        // Group students by section
        $grouped_students = array();
        foreach ($studentList as $student) {
            $section_key = $student->class . '-' . $student->section_id; // or just $student->section_id
            $grouped_students[$section_key]['section_name'] = $student->class . " (" . $student->section . ")";
            $grouped_students[$section_key]['students'][] = $student;
        }

        $data['grouped_studentList'] = $grouped_students;
    ...
    public function subjectstudent()
    {
        ...
        $resultlist = $this->cbseexam_exam_model->get_markexamstudents($data['timetable_id'], $data['class_id'], $data['section_id']);
        $data['resultlist'] = $resultlist;
        
        // Get class and section lists for filters
        $data['classlist'] = $this->class_model->get();
        $data['sectionlist'] = array();
        if ($data['class_id']) {
            $data['sectionlist'] = $this->section_model->getClassBySection($data['class_id']);
        }
    ...
    public function entrymarks()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $result = $this->cbseexam_exam_model->addresultmark_data($insert_array, $cbse_exam_timetable_id);
            
            if ($result) {
                $array = array('status' => '1', 'error' => '', 'message' => $this->lang->line('success_message'));
            } else {
                $array = array('status' => '0', 'error' => '', 'message' => 'Failed to save marks');
            }
            echo json_encode($array);
        }
    }
