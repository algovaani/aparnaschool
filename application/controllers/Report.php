...
class Report extends Admin_Controller
{
    ...
    //     $result = $this->student_model->admission_reportNew($searchterm, $carray, $condition);

    //     $resultlist = json_decode($result);
    //     $dt_data    = array();
    //     echo "<pre>";
    //     print_r($resultlist->data);
    //     echo "</pre>";exit();
    //     if (!empty($resultlist->data)) {
    ...
    //             $row[] = ($student->admission_fee_amount > 0) ? $currency_symbol . amountFormat($student->admission_fee_amount) : "-";
    ...
    if (!empty($resultlist->data)) {
        foreach ($resultlist->data as $resultlist_key => $student) {
            ...
            // Calculate net admission fee (amount - discount)
            $fee_details = [];
            $total_paid = 0;
            $total_discount = 0;
        ...
        // print_r($resultlist->data);
        // echo "</pre>";exit();
            // Process fee_details if it exists
            if (!empty($student->fee_details)) {
                $fee_parts = explode('||', $student->fee_details);
                
                foreach ($fee_parts as $part) {
                    if (empty($part)) continue;
                    
                    $detail = json_decode($part, true);
                    if (!is_array($detail)) continue;
                    
                    foreach ($detail as $item) {
                        // Check if payment date matches admission date
                        if (date('Y-m-d', strtotime($item['date'])) == $student->admission_date) {
                            // Always count the payment amount
                            if (isset($item['amount'])) {
                                $total_paid += (float)$item['amount'];
                            }
                            
                            // Check for discount
                            if (isset($item['amount_discount']) && (float)$item['amount_discount'] > 0) {
                                $total_discount = (float)$item['amount_discount'];
                                 $discount_found = true;
            ...
            $net_admission_fee = $student->admission_fee_amount - $total_discount;
            ...
            $fee_display = "-";
            if ($student->admission_fee_amount > 0) {
                $fee_display = $currency_symbol . amountFormat($student->admission_fee_amount);
                
                // Only show discount if there is any
                if ($total_discount > 0) {
                    $fee_display .= "<br><small>(-" . $currency_symbol . amountFormat($total_discount) . " discount)</small>";
                }
                
                // Show net amount
                $fee_display .= "<br><strong>Net: " . $currency_symbol . amountFormat($net_admission_fee) . "</strong>";
            }
            $row[] = $fee_display;
    ...
    /**
     * Issued Certificate Report - Certificate wise, Class wise filter, list all students
     */
    public function issued_certificate_report()
    {
        ...
        $this->session->set_userdata('sub_menu', 'admin/report/issued_certificate_report');
        ...
        $data['section_id']      = '';
        $data['certificate_id']  = '';
        $data['title']           = $this->lang->line('issued_certificate_report');
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == true) {
                ...
                if (!empty($class_id) && !empty($section_id)) {
                    ...
                    $data['filter_title'] = $this->lang->line('all_students');
                }
            }
        }
        ...
        $data['sch_setting'] = $this->sch_setting_detail;
        $this->load->view('layout/header', $data);
        $this->load->view('admin/report/issued_certificate_report', $data);
        $this->load->view('layout/footer', $data);
    }

    /**
     * Fees due report: students who have not paid fees for 60+ days, with months unpaid
     */
    public function fees_due_60_days_report()
    {
        if (!$this->rbac->hasPrivilege('student_fees', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/report/fees_due_60_days_report');
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $data['class_id'] = $class_id;
            $data['section_id'] = $section_id;
            $data['resultlist'] = $this->studentfee_model->getStudentsWithDueFeesOver60Days($class_id ?: null, $section_id ?: null);
        } else {
            $data['resultlist'] = $this->studentfee_model->getStudentsWithDueFeesOver60Days(null, null);
        }

        $data['sch_setting'] = $this->sch_setting_detail;
        $data['currency_format'] = $this->customlib->getSchoolCurrencyFormat();
        $this->load->view('layout/header', $data);
        $this->load->view('admin/report/fees_due_60_days_report', $data);
        $this->load->view('layout/footer', $data);
    }
}
