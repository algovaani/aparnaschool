                        ...
                        if (in_array("whatsapp", $sms_mail)) {
                            ...
                            if ($user_mail_value['app_key'] != "") {
                               $client = new Client();
                                ...
                                $response = $client->get('https://web.mastermindz.in/send-message', [
                                            'query' => [
                                                'api_key' => $sms_detail->authkey,
                                                'sender' => $sms_detail->contact,//$this->config->item('whatsapp_sender_number'),
                                                'number' => $user_mail_value['mobileno'],
                                                'message' => $message
                                            ]
                                        ]);
                            }
            ...
            if ($send_type == 'send_now') {
                if (!empty($user_array)) {
                    ...
                    foreach ($user_array as $user_mail_key => $user_mail_value) {
                        ...
                        if (in_array("whatsapp", $sms_mail)) {
        //                     echo "success2222===".$user_mail_value['mobileno'];
        // exit();
                            $push_array = array(
                                'title' => $message_title,
                                'body'  => $message,
                            ...
                            // if ($user_mail_value['app_key'] != "") {
                               $client = new Client();
                                // $sms_detail = $this->_CI->smsconfig_model->getActiveSMS();
                                $response = $client->get('http://web.mastermindz.in/send-message', [
    'query' => [
        'api_key' => "eJJ1Hd77ENzbbV9tmRqHE0lDnUYz7y",
        'sender'  => "918989484928",
        'number'  => "916375649508",
        'message' => "helloo"
    ],
