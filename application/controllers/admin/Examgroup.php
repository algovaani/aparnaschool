...
class Examgroup extends Admin_Controller
{
    ...
    public function exportformatcbse()
    {
        $this->load->helper('download');
        
        // Get exam details from POST data
        ...
        if (!$exam_id || !$timetable_id) {
            // Fallback to static file if no exam data provided
            $filepath = "./backend/import/import_marks_sample_file_cbse.csv";
            $data = file_get_contents($filepath);
            $name = 'import_marks_sample_file_cbse.csv';
            force_download($name, $data);
            return;
        }
        ...
        $resultlist = $this->cbseexam_exam_model->get_markexamstudents($timetable_id, $class_id, $section_id);
        
        // Get assessment types for the subject
        $exam_assessment_types = $this->cbseexam_exam_model->get_exam_subject_assessment_types($examdetails['cbse_exam_assessment_id'], $timetable_id);
        
        // Count total subjects (assessment types)
        $total_subjects = 0;
        foreach ($exam_assessment_types as $assessment) {
            if (!is_null($assessment->cbse_exam_timetable_assessment_type_id)) {
                $total_subjects++;
            }
        }
        
        // Generate CSV content
        $csv_content = "adm_no,status,Total subject marks";
        ...
        foreach ($resultlist as $student) {
            $csv_content .= $student['admission_no'] . ",1," . $total_subjects;
            
            // Add marks columns (empty for sample)
            for ($i = 1; $i <= $total_subjects; $i++) {
                $csv_content .= ",0";
            }
            
            // Add attendance columns (1 = present)
            for ($i = 1; $i <= $total_subjects; $i++) {
                $csv_content .= ",1";
            }
            
            $csv_content .= ",Sample note\n";
        }
        
        $name = 'import_marks_sample_file_cbse_' . date('Y-m-d_H-i-s') . '.csv';
        force_download($name, $csv_content);
    }

    public function exportformatcbse1()
        {
            $this->load->helper('download');
            $filepath = "./backend/import/import_marks_sample_file_cbseAllMarks.csv";
            $data     = file_get_contents($filepath);
            $name     = 'import_marks_sample_file_cbse.csv';
            force_download($name, $data);
        }

    public function uploadfilecbse()
    {
                ...
                if (isset($_FILES["file"]) && !empty($_FILES['file']['name']) && $_FILES["file"]["size"] > 0) {
                    ...
                    while (($column = fgetcsv($file, 10000, ",")) !== false) {
                        ...
                        if (trim($column[0]) != "" && trim($column[1]) != "" && trim($column[2]) != "") {
                            ...
                            for ($i = 1; $i <= $total_subjects; $i++) {
                                $data['marks'.$i] = number_format($column[$b], 2, '.', '');
                                $b++;
                            }

                            for ($i = 1; $i <= $total_subjects; $i++) {
                                $data['attands'.$i] = $column[$b];
                                $b++;
                            }

                            $data['note'] = $this->encoding_lib->toUTF8($column[$b]);
                            $return_array[] = json_encode($data);
                        }
                    }
                }
                $array = array('status' => '1', 'error' => '', 'student_marks' => $return_array,'total_subjects'=>$total_subjects);
                echo json_encode($array);
            }
            //=============
        }
   ...
   public function addexam($id)
    ...
    public function uploadfilecbse1()
{
    $this->form_validation->set_error_delimiters('', '');
    $this->form_validation->set_rules('file', $this->lang->line('image'), 'callback_handle_upload');
    ...
    if ($this->form_validation->run() == false) {
        ...
        $return_array = array();
        $total_subjects = 1; // Default value
        ...
        if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
            ...
            if (is_uploaded_file($file) && is_readable($file)) {
                ...
                if ($file_handle !== false) {
                    ...
                    while (($column = fgetcsv($file_handle, 10000, ",")) !== false) {
                        $v++;
                        if ($flag) {
                            $flag = false;
                            continue;
                        ...
                        if ($v == 2) {
                            $total_subjects = number_format($column[2]);
                        }

                        if (count($column) >= $total_subjects) {
                            $data = array(
                                'adm_no' => $column[0],
                                'status' => $column[1]
                            );
                            ...
                            for ($i = 0; $i < $total_subjects; $i++) {
                                $data['marks' . ($i + 1)] = number_format($column[$i + 3], 2, '.', '');
                            }
                ...
                // Handle file not uploaded or not readable
            }
        }
