...
class Generatecertificate extends Admin_Controller
{
    ...
    public function generatemultiple()
    {
        ...
        foreach ($data['students'] as $key => $value) {
            ...
            $fee_paid          = 0;
            $studentDueFee = array();
            // echo '<pre>';print_r($student_due_fee); exit();
            foreach ($student_due_fee as $key1 => $fee) {
                foreach ($fee->fees as $fee_key => $fee_value) {
                    // new condition for get only tuition fee code by #as
                    if(strpos(strtolower($fee_value->name),"tuition fee")){
                        $data['students'][$key]->tutionFee1 =   $fee_value->amount;
                    };
                    // new condition for get only tuition fee code by #as
                    if (!empty($fee_value->amount_detail) && (strpos(strtolower($fee_value->name),"tuition fee"))) {
