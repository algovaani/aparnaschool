            ...
            if (!empty($user_array)) {
                if ($send_sms) {
                    ...
                    if ($sms_detail && $sms_detail->type == 'custom') {
                        ...
                        $bulk_result = $this->customsms->sendBulkSMS($user_array, $message, 20);
                        
                        // Log the results
                        $this->load->model('message_log_model');
                        $log_data = array(
                            'message_id' => 0, // Will be updated after message is saved
                        ...
                        // Use regular SMS gateway for other providers
                        foreach ($user_array as $user_mail_key => $user_mail_value) {
                            if ($user_mail_value['mobileno'] != "") {
                                ...
                                if ($user_mail_key < count($user_array) - 1) {
                    ...
                    foreach ($user_array as $user_mail_key => $user_mail_value) {
                        if (in_array("sms", $sms_mail)) {
                            if ($user_mail_value['mobileno'] != "") {
                                ...
                                // Add small delay for SMS providers
                                if ($user_mail_key < count($user_array) - 1) {
                                    sleep(2); // 2 seconds delay for regular SMS
                                }
                            }
