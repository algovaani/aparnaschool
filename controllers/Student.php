...
class Student extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array("timeline_model", "Student_model", "Subject_model","student_edit_field_model", 'transportfee_model', 'marksdivision_model', 'module_model'));
    ...
    public function view($id)
    {
        ...
        if (isset($student['admission_no']) && !empty($student['admission_no'])) {
            $admissionNo = $student['admission_no'];
            // barCode
            $filePath = FCPATH . 'uploads/student_id_card/barcodes/' . $id . '.png';
            if (!file_exists($filePath)) {
                // Assuming this returns HTML/base64 barcode
                $uploadPath = FCPATH . '/uploads/student_id_card/barcodes/';
                $qrCode = $this->customlib->generatebarcode($students_value->admission_no,$id);
            }
        }
        $transport_fees = [];
    ...
    public function edit($id)
    {
        ...
        $student_subject      = $this->Subject_model->get();
        $data['subject_group'] = $student_subject ;
        //subject_group
        $data['adm_auto_insert'] = $this->sch_setting_detail->adm_auto_insert;
        $data['genderList']      = $genderList;
        $session                 = $this->setting_model->getCurrentSession();
        ...
        if ($this->form_validation->run() == false) {
            ...
            // echo "</pre>";
            // exit();
            // print_r($this->input->post('subject_choose'));die;
           ...
           $sub_id = $this->input->post('subject_name');
           $subjects = $this->Subject_model->getDataByMultipleIds($sub_id);
           
           $names = array_map(function($item) {
                return $item->name;
            }, $subjects);
            $sub_name = implode(', ', $names);
            $data = array(
                'id'                => $id,
                'firstname'         => $this->input->post('firstname'),
                'rte'               => $this->input->post('rte'),
                ...
                'crft_sub'        => $this->input->post('crft_sub'),
                'core_sub'        => $this->input->post('core_sub'),
                'first_fee_paid_date'        => $this->input->post('first_fee_paid_date'),
                'last_fee_paid_date'        => $this->input->post('last_fee_paid_date'),
                'concession_reciept'        => $this->input->post('concession_reciept'),
                'ncc_cadet'        => $this->input->post('ncc_cadet'),
                'game_play'        => $this->input->post('game_play'),
                'cunduct'        => $this->input->post('cunduct'),
                'anul_chrg_colct'        => $this->input->post('anul_chrg_colct'),
                'tot_school_days'         => $this->input->post('tot_school_days'),
                'pupil_attend_days' => $this->input->post('pupil_attend_days'),
